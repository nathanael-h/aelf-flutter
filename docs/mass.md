...
`sequence` and `solemnBlessingList` are `List<HymnEntry>?`, not literal text — see "Hymn/blessing hydration" below.
...
massExport(CelebrationContext)
  -> Future<Mass>
     ...
     7. Resolve sequence/solemnBlessingList codes into content
          (resolveOfficeContent(hymns: ..., blessings: ..., dataLoader: ...))
```
...
Almost every piece of Mass content is literal text already in the YAML, not a code reference into a shared library — but two fields are the exception, and are resolved through the exact same mechanism every other office's `hymn:` field uses:
...
Only Easter (Sunday and the Octave weekdays) has non-null `sequence`/`solemnBlessingList` data so far; every other day still has both as `null`, same as before this mechanism existed.
...
- **The proper sequence gets its own conditional tab, "Séquence".** Shown only when `massData.sequence` is non-empty — rare, a handful of days a year (Easter and its Octave, Pentecost). Positioned right before the Gospel (the last reading part — see `_readingPartLabels`'s doc comment), in both tab and scroll mode, since the sequence is sung after the second reading and before the Gospel acclamation. Reuses `HymnsTabWidget` — the same selector every other office's `hymn:` tab uses — passing `title: 'Séquence'` to override its default "Hymnes" section title (`HymnsTabWidget` gained an optional `title` parameter for this; every other caller keeps the old default).
