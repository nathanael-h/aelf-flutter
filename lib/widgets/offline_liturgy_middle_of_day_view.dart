...
class _MiddleOfDayOfficeViewState extends State<MiddleOfDayOfficeView>
  ...
  Future<void> _onPrecedenceOverridden(String key, int? newPrecedence) async {
    final definition = widget.middleOfDayList[key];
    debugPrint(
        '[PrecedenceDebug][MiddleOfDay] key=$key | originalPrecedence=${definition?.precedence} | newPrecedence=$newPrecedence | commonList=${definition?.commonList}');
      ...
      return _OfficeDisplay(
        ...
        definition: _selectedDefinition!.copyWith(
          ...
          precedence: context
                  .read<SelectedCelebrationState>()
                  .getPrecedenceOverride(_celebrationKey!) ??
              _selectedDefinition!.precedence,
        ),
...
class _IntroductionTab extends StatelessWidget {
  const _IntroductionTab(
      {required this.definition, required this.calendar, required this.date});
